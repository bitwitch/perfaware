...
typedef enum {
	...
	BP_PATTERN_EVERY3,
	BP_PATTERN_EVERY4,
	BP_PATTERN_CRT_RANDOM,
	BP_PATTERN_OS_RANDOM,
	BP_PATTERN_COUNT,
} BranchPredictionPattern;

static char *pattern_names[BP_PATTERN_COUNT] = {
	[BP_PATTERN_ALWAYS] = "Always Taken",
	[BP_PATTERN_NEVER] = "Never Taken",
	[BP_PATTERN_EVERY2] = "Every 2nd Taken",
	[BP_PATTERN_EVERY3] = "Every 3rd Taken",
	[BP_PATTERN_EVERY4] = "Every 4th Taken",
	[BP_PATTERN_CRT_RANDOM] = "CRT Random",
	[BP_PATTERN_OS_RANDOM] = "OS Random",
};

static void fill_buffer_by_pattern(BranchPredictionPattern pattern, U8 *buf, U64 buf_size) {
	if (pattern == BP_PATTERN_OS_RANDOM) {
		if (!os_random_bytes(buf, buf_size)) {
			fprintf(stderr, "error: os_random_bytes failed\n");
			exit(1);
		}
	} else {
		for (U64 i=0; i<buf_size; ++i) {
			switch (pattern) {
			case BP_PATTERN_ALWAYS:
				buf[i] = 1;
				break;
			case BP_PATTERN_NEVER:
				buf[i] = 0;
				break;
			case BP_PATTERN_EVERY2:
				buf[i] = (i % 2) == 0;
				break;
			case BP_PATTERN_EVERY3:
				buf[i] = (i % 3) == 0;
				break;
			case BP_PATTERN_EVERY4:
				buf[i] = (i % 4) == 0;
				break;
			case BP_PATTERN_CRT_RANDOM:
				buf[i] = rand();
				break;
			default: 
				assert(0);
...
void simple_branch(U64 count, U8 *data);
#pragma comment (lib, "simple_branch")

int main(int argc, char **argv) {
	os_metrics_init();

	RepetitionTester testers[BP_PATTERN_COUNT] = {0};
	U32 seconds_to_try = 10;
	U64 cpu_timer_freq = estimate_cpu_freq();

	printf("CPU freq: %fGHz\n", (double)cpu_timer_freq / (double)(1000*1000*1000));

	U64 buf_size = 1024 * 1024 * 1024; // 1GB
	U8 *buf = xmalloc(buf_size);

	for (;;) {
		for (int pattern = 0; pattern < BP_PATTERN_COUNT; ++pattern) {
			RepetitionTester *tester = &testers[pattern];

			fill_buffer_by_pattern(pattern, buf, buf_size);

			printf("\n--- %s ---\n", pattern_names[pattern]);
			new_test_wave(tester, buf_size, cpu_timer_freq, seconds_to_try);

			while (is_testing(tester)) {
				begin_time(tester);
				simple_branch(buf_size, buf);
				end_time(tester);
				count_bytes(tester, buf_size);
			}
		}
	}
